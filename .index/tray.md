...
| [src/app/tray.rs](../tray_src/src/app/tray.rs)     | Menu-bar icon + menu: `TrayCommand { ShowWindow{rect}, Quit }`, `install`, `subscription` (icon + menu event threads), `build_icon` (procedural glyph), `main_screen_scale` (NSScreen), `round_window` (CALayer rounded corners, no shadow), `focus_window` (activate accessory app + make key), `mouse_position_top_left` (NSEvent cursor, flipped)                          |
