...
| [src/app/chrome.rs](../main_src/src/app/chrome.rs)           | macOS window chrome: `apply` — transparent system title bar + full-size content view (keeps native traffic lights) so the custom title bar can draw over them. `#[cfg(macos)]`-gated, non-macOS stub. Applied on `WindowOpened` |
...
| File                                                       | Purpose                                                                                                                          |
| ---------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| [ui/mod.rs](../main_src/src/app/ui/mod.rs)             | Top-level composer: `view_root` (title bar + sidebar + content + status bar + modals), `workspace_or_prompt`, `status_bar`, `list_sidebar`; the whole window is one solid `shell` surface |
